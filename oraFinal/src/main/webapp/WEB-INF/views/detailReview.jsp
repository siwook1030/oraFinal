...
<meta name="_csrf" content="${_csrf.token}" />
	...
	<style type="text/css">
		...
		.replyNickname { /* margin-top: 3px;  */font-size: 14px; padding-left: 5px; width: 230px; vertical-align: middle; border: 1px solid yellow; }
		.regdate {
			flex-grow: 1;
			flex-basis: 90%;
			/* text-align: right; */
		...
		.replyContent { padding-left: 30px; font-size: 14px; border: 1px solid green; }
		/* 댓글창 */
		.replyToReplyArea {  border: 1px solid purple; }
		.textareaContainer > textarea { width: 100%; height: 130px; }
		.btnContainer img { width: 30px; }
		.btnReply { font-size: 13px; }
</style>
...
var $replyTextArea = $("<textarea></textarea>");	// 대댓글을 위한 textarea 전역변수
...
/* 닉네임 */		let $div_c1_c2 = $("<div></div>").append(item.nickName).addClass("replyNickname");
/* 작성일자 */		let $div_c1_c3 = $("<div></div>").text(item.date_diff_str).addClass("regdate"); // css적용을 위한 클래스
				...
				if(login_id === rr_id) {	// 로그인id와 댓글작성id가 일치할 경우 수정,삭제 아이콘 보이기
					let $img_mod = $("<img>").attr("src", "icons/eraser.png");
					let $btn_mod = $("<div></div>").attr("title", "수정").append($img_mod);
					//let $a_mod = $("<a></a>").attr("href", "").append($btn_mod);
					
					let $img_del = $("<img>").attr("src", "icons/remove.png");
					let $btn_del = $("<div></div>").attr("title", "삭제").append($img_del);
					//let $a_del = $("<a></a>").attr("href", "").append($btn_del);
					
					$div_c3_c1.append($div_c1_c3, $btn_mod, $btn_del);
				}
				if(login_id !== "") {		// 현재 로그인한 사용자일 경우 댓글아이콘 보이기
					...
					let $btn_rep = $("<div></div>").html("답글달기").addClass("btnReply");
...
<body>
	<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
		<div class="container">
			<a style="font-family: NanumSquareRoundR; font-size: 30px;" class="navbar-brand" href="/mainPage">
		...
		<div class="container">
			<div class="row no-gutters slider-text js-fullheight align-items-center justify-content-center">
				<div class="col-md-9 ftco-animate pb-0 text-center">
					...
					<h1 class="mb-3 bread">라이딩 후기 상세</h1>
				</div>
			</div>
		</div>
    </section>
	...
	<section class="ftco-section ftco-agent">
		<div class="container">
			...
			<hr>
			<!-- 댓글 목록 영역 -->
			<div id="replyArea" style="border: 3px solid pink;"></div>
			<hr>
			<!-- 본문 댓글 영역 -->
			<c:if test="${m != null }">
